 ...
 */
function analyzeReplayWithPython(
  replayPath: string,
 ...
 */
async function storeReplayInDatabase(
  ...
  const [replayRecord] = await db
    ...
    .values({
      ...
      playedAt: game_info.played_at
        ? new Date(game_info.played_at * 1000)
        : null,
    })
    .returning();
  ...
  for (const playerData of playersData) {
    ...
    if (!replayPlayerRecord) {
      throw new Error(
        `Failed to insert replay-player record for ${player.name}`,
      );
    }
 ...
 */
export async function getAnalyzedReplays(): Promise<
  Array<{
    id: number;
    slug: string;
    filename: string;
 ...
 */
export async function getReplayBySlug(
  slug: string,
): Promise<ReplayAnalysisResult | null> {
  ...
  return {
    ...
    game_info: {
      ...
      played_at: existingReplay[0]!.playedAt
        ? Math.floor(existingReplay[0]!.playedAt.getTime() / 1000)
        : 0,
    },
 ...
 */
export async function getReplayTimeSeriesBySlug(
  slug: string,
  ...
  return snapshots.map((snapshot) => ({
    timestamp: snapshot.timestamp,
    players: JSON.parse(snapshot.snapshotData) as Record<
      string,
      {
        name: string;
        race: string;
